...
class SpinModel():
  ...
  def hp_heisenberg(self,fun=None,d=None):
    return hp_heisenberg(self,fun=fun,d=d)
...
def hp_heisenberg(sm,fun=None,d=None):
  ...
  def c2h(m0):
    """Couplings to HP-hamiltonian"""
    # SzSz terms
    # There are two matrices, one that renormalizes onsite energies
    # and another one that creates hoppings
    iden = np.identity(m0.shape[0],dtype=np.complex) 
    jdiag = np.array([np.sum(im) for im in m0])*spins # sum rows
    mons = csc_matrix((jdiag,(inds,inds)),shape=m0.shape).todense() #diagonal
#    print("SzSz",mons)
    # S+ S- terms
    # matrix qith sqrt(s_is_j)
    sij = np.array([[np.sqrt(si*sj) for si in spins] for sj in spins])
    jij = np.array(m0) # matrix with couplings
#    for i in range(ns): jij[i,i] = 0.0 # set diagonal to zero
    mhop = -jij*sij # add contribution
#    print("S+S-",mhop)
    return (np.matrix(mons),np.matrix(mhop)) # return matrices
  def c2h(m0):
