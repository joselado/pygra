...
# run the calculation
  ...
  def didv(self,energy=0.0,delta=0.0001):
    delta = self.delta
    return didv(self,energy=energy,delta=delta) # return value
  def block2full(self):
    """Put in ful form"""
    block2full(self)
...
def central_dos(hetero,energies=[0.0],num_rep=100):
   ...
   if hetero.block_diagonal: ht.block2full()
...
def block2full(ht):
  ...
  into the full form"""
  if not ht.block_diagonal: return # stop
  ht.block_diagonal = False # set in false from now on
  nb = len(ht.central_intra) # number of blocks
  lc = [coo_matrix(ht.central_intra[i][i]*0.) for i in range(nb)]
  rc = [coo_matrix(ht.central_intra[i][i]*0.) for i in range(nb)]
  lc[0] = ht.left_coupling
  rc[nb-1] = ht.right_coupling
  from scipy.sparse import vstack
  ht.left_coupling = vstack(lc).todense()
  ht.right_coupling = vstack(rc).todense()
  ht.central_intra = bmat(ht.central_intra).todense() # as dense matrix
...
def didv(ht,energy=0.0,delta=0.00001):
  ...
  if ht.has_eh: # for systems with electons and holes
    ...
    return G
  else: return landauer(ht,energy=energy,delta=delta) # call usual landauer 
