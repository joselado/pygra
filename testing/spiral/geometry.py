...
class geometry:
  ...
  a1 = np.array([1.0,0.0])  # first vector to the nearest cell
  a2 = np.array([0.0,1.0])  # first vector to the nearest cell
  ...
  def supercell(self,nsuper):
    ...
    if self.dimensionality==1:
      g = supercell1d(self,nsuper)
    if self.dimensionality==2:
      g = supercell2d(self,n1=nsuper,n2=nsuper)
    return g
...
def bipartite_square_ribbon(n):
  """ Creates the hamiltonian of a bipartite square ribbon lattice"""
  from numpy import array
  x=array([0.0 for i in range(4*n)]) # create x coordinates
  y=array([0.0 for i in range(4*n)])  # create y coordinates
  for i in range(n): # create the diffeent atoms
    y[4*i],x[4*i] = float(2*i),-0.5 
    y[4*i+1],x[4*i+1] = float(2*i),0.5 
    y[4*i+2],x[4*i+2] = float(2*i+1),0.5 
    y[4*i+3],x[4*i+3] = float(2*i+1),-0.5 
  y=y-sum(y)/float(4*n) # shift to the center
  g = geometry() # create geometry class
  g.x = x  # add to the x atribute
  g.y = y  # add to the y atribute
...
def supercell2d(g,nx=1,ny=1):
  """ Creates a supercell for a 2d system"""
  y = g.y
  x = g.x
  if not g.dimensionality==2:
    raise
...
def honeycomb_lattice():
  ...
  g.a1 = np.array([3./2.,np.sqrt(3.)/2]) # first lattice vector
  g.a2 = np.array([-3./2.,np.sqrt(3.)/2]) # second lattice vector
