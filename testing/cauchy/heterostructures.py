...
#    gc.set_finite()
  ...
  def plot_central_dos(self,energies=[0.0],num_rep=100,
    ...
    return plot_central_dos(self,energies=energies,num_rep=num_rep,
                      ...
                      max_error=max_error)
  def plot_landauer(self,energy=[0.0],delta=0.001):
    """ Plots the density of states by using a 
    green function approach"""
    return plot_landauer(self,energy=energy,delta=delta)
  def landauer(self,energy=[0.],delta=0.0001):
    """ Return the Landauer transmission"""
    return landauer(self,energy=energy,delta=delta)
  ...
  def eigenvalues(self,numeig = 10,effective=False,gf=None,full=False):
...
def landauer(hetero,energy=0.0,delta = 0.001,error=0.00001,do_leads=True,
   ...
   try: # if it is a list
     return [landauer(hetero,energy=e,delta=delta,error=error,
                      do_leads=do_leads,gr=gr,gl=gl) for e in energy]
...
def plot_landauer(ht,energy=[0.0],delta=0.001):
   """ Plots the density of states and Landauer transmission
    by using a 
    green function approach"""
   # get the transmission
   trans = landauer(ht,energy=energy,delta=delta)
   # plot the figure
   fig = py.figure() # create figure
