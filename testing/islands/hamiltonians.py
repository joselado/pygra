...
# dimensionality of the system
  ...
  def check(self):
    ...
    if np.max(np.abs(hh[i,j])) > 0.000001:
  ...
  def add_rashba(self,c):
    ...
    g = self.geometry
    self.intra += rashba(g.x,g.y,c=c)
    if g.dimensionality==1:  # one dimensional
      self.inter += rashba(g.x+g.celldis,g.y,x2=g.x,y2=g.y,c=r)
    if g.dimensionality==2:  # two dimensional
      self.tx += rashba(g.x+g.a1[0],g.y+g.a1[1],x2=g.x,y2=g.y,c=c)
      self.ty += rashba(g.x+g.a2[0],g.y+g.a2[1],x2=g.x,y2=g.y,c=c)
      self.txy += rashba(g.x+g.a1[0]+g.a2[0],g.y+g.a1[1]+g.a2[1],x2=g.x,y2=g.y,c=c)
      self.txy += rashba(g.x+g.a1[0]-g.a2[0],g.y+g.a1[1]-g.a2[1],x2=g.x,y2=g.y,c=c)
...
#      rs = [[x,y],[x+g.celldis,y]]
    if self.dimensionality==2:  # two dimensional
      rs = [] # all the cells
...
def rashba(x1,y1,x2=None,y2=None,c=0.):
...
#    print "Shifting FERMI energy, input is NUMBER"
  ...
  if callable(fermi):  # if fermi is a function
    ...
    if h.has_eh:  # change signs for electrons and holes
      for i in range(n/2):      
        intra[2*i,2*i] += fermi(x[i/indd],y[i/indd])
        intra[2*i+1,2*i+1] += -fermi(x[i/indd],y[i/indd])
    else:  # same sign for everyone
      for i in range(n):
        intra[i,i] += fermi(x[i/indd],y[i/indd])
...
def add_peierls(h,mag_field=0.0):
  ...
  if h.dimensionality>1: # if bigger dimensionl
    raise
