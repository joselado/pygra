...
# dimensionality of the system
  ...
  def add_kane_mele(self,t):
    ...
    g = self.geometry  
    (intra_soc, inter_soc) = kane_mele(g.x,g.y,celldis = g.celldis,
                                        lambda_soc = t)
  ...
  def generate_spin_spiral(self,vector=np.array([1.,0.,0.]),angle=0.):
    from rotate_spin import global_spin_rotation as gsr
    self.inter = gsr(self.inter,vector=vector,angle=angle,spiral=True)
...
def kane_mele(x,y,first = 1.0, celldis = 3.0,lambda_soc=0.0,mag_field=0.0):
  """ Computes the Kane-Mele matrix,
      input are x and y coordinates,
      distance to first neighbors,
      distance to nearest cell,
  ...
  from increase_hilbert import m2spin
  from numpy import array
  def haldane(value,minus=False): # calculate Haldane hopping
...
# total hopping and onsite
  ...
  (intra_up,inter_up) = haldane(lambda_soc,minus=False)  # up component
  (intra_dn,inter_dn) = haldane(lambda_soc,minus=True)  # up component
...
# routine to obtain haldane hopping to second neighbors
def get_hal_hop_sec_neigh(x1,y1,x2,y2,xm,ym,mag_field=0.0,value=0.0):
  from scipy.sparse import coo_matrix, bmat
  from numpy import array, matrix, sqrt
  s3=sqrt(3.0)
  n=len(x1)
...
def nambu(m,coupling=None):
  ...
  if not coupling == None:
    ...
    mo[0][1] = coupling.H # hole-electron 
