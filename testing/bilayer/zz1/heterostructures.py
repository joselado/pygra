...
#    gc.set_finite()
  ...
  def plot_central_dos(self,energies=[0.0],num_rep=100,
    ...
    return plot_central_dos(self,energies=energies,num_rep=num_rep,
                      ...
                      max_error=max_error)
  def plot_landauer(self,energies=[0.0],gf=None):
    """ Plots the density of states by using a 
    green function approach"""
    return plot_landauer(self,energies=energies,gf=gf)
  ...
  def eigenvalues(self,numeig = 10,effective=False,gf=None):
    """ Calculates eigenvalues of the central part """
    return eigenvalues(self,numeig=numeig,effective=effective,gf=gf)
...
def landauer(hetero,energies=0.0,gf=None):
...
# dimension of the central part
   ...
   for energy in energies: # loop over energies
     # use initialized green functions
#     if green_guess:
#
#       gr = hetero.right_green
#       gl = hetero.left_green
#     else:
     gr = None
     gl = None
     intra = hetero.right_intra
...
def plot_landauer(ht,energies=[0.0],gf=None):
   """ Plots the density of states and Landauer transmission
    by using a 
    green function approach"""
   # get the transmission
   trans = landauer(ht,energies=energies,gf=gf)
   # plot the figure
   fig = py.figure() # create figure
...
#   sdos.set_xlim([min(energies),max(energies)])
   ...
   strans.plot(energies,trans,color="green") # create the plot
   strans.fill_between(energies,0,trans,color="green")
   strans.set_ylim([0.0,max(trans)+0.5])
   strans.set_xlim([min(energies),max(energies)])
...
def eigenvalues(hetero,numeig=10,effective=False,gf=None):
...
def effective_central_hamiltonian(hetero,energy=0.0,write=False,gf=None):
   """ Plots the local density of states in the central part"""
   from green import dyson
   from hamiltonians import is_number
   if not hetero.block_diagonal:
   ...
   intra = hetero.right_intra
   inter = hetero.right_inter
   gr = dyson(intra,inter,is_sparse=hetero.is_sparse,gf=gf)
   hetero.right_green = gr # save green function
   # left green function
   intra = hetero.left_intra
   inter = hetero.left_inter
   gl = dyson(intra,inter,is_sparse=hetero.is_sparse,gf=gf)
   hetero.left_green = gl # save green function
   # save green functions
   hetero.write_green()
   print "Saved green functions"
