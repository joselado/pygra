...
# dimensionality of the system
  ...
  def get_bands(self,nkpoints=100,use_lines=False,kpath=None,operator=None,
    ...
    if self.dimensionality==1: # for 0d and 1d system
      return get_bands1d(self,nkpoints=nkpoints,operator=operator,
                         num_bands=num_bands)
  ...
  def add_pwave(self,delta=0.0,mu=0.0,phi=0.0):
    """ Adds spin conserving first neighbor electron hole pairing"""
    add_pwave_electron_hole_pairing(self,delta=delta,mu=mu,phi=phi)
   ...
   # self.intra = add_swave(self.intra,delta=delta,mu=mu,phi=phi)
    ...
    if self.is_multicell: # for multicell hamiltonians
      for i in range(len(self.hopping)): # loop over hoppings
        self.hopping[i].m = nambu(self.hopping[i].m) # put in nambu form
...
# add element to the matrix
         mat[i1,i2] += 1j*vec_chi(r1-rm,r2-rm)*fac 
  return mat

# routine to obtain first hoppings
def get_hop_first_neigh(x1,y1,x2,y2):
  n=len(x1)
  ...
  for p in pairs: # loop over pairs
    mat[p[0],p[1]] = 1.0 # element of the matrix
  return mat
...
def get_bands1d(h,nkpoints=100,operator=None,num_bands=None):
...
def add_sublattice_imbalance(h,mass):
  ...
  massterm = csc_matrix((data,(rows,rows)),shape=(n,n))
  if not h.is_sparse: massterm = massterm.todense() # dense matrix
  h.intra = h.intra + massterm
...
#          intra[2*i,2*i] += mass*ab(i)
          rows.append(2*i)
          cols.append(2*i)
          data.append(mass*ab(i)) # up
          rows.append(2*i+1)
          cols.append(2*i+1)
          data.append(-mass*ab(i)) # down
        else: raise
    elif callable(mass):  # if mass is a function
      x = h.geometry.x
      y = h.geometry.y
      for i in range(natoms):
        if d=="z":
          rows.append(2*i)
          cols.append(2*i)
          data.append(mass(x[i],y[i])*ab(i)) # up
          rows.append(2*i+1)
          cols.append(2*i+1)
          data.append(-mass(x[i],y[i])*ab(i)) # down
        else: raise
    else:  # something else error
      print("unrecognized AF option")
      raise
    maf = csc_matrix((data,(rows,cols)),shape=h.intra.shape)
    if not h.is_sparse: maf = maf.todense() # dense matrix
    h.intra = h.intra + maf # add AF
  else:
...
def add_pwave_electron_hole_pairing(h,delta=0.0,mu=0.0,phi=0.0):
  ...
  zero = h.intra*0.  # intra cell potential
  r = h.geometry.r # r position
  g = h.geometry  # geometry object
  from scipy.sparse import coo_matrix as coo
  from scipy.sparse import bmat
  deltac = delta*np.exp(1j*phi*np.pi) # complex SC order parameter
  # intracell contribution
  def get_pwave_matrix(r1,r2):
    intra_sc = 0.0*zero  # intracell pairing potential
    x1 = r1[:,0]
    y1 = r1[:,1]
    x2 = r2[:,0]
    y2 = r2[:,1]
# get intra pairs of first neighbors
    pairs = find_first_neighbor(x1,y1,x2,y2) 
    for p in pairs: # loop over intracell pairs
   # geometric phase, dependent on bonding orientation
      gp = np.arctan2(y1[p[0]]-y2[p[1]],x1[p[0]]-x2[p[1]]) 
    ...
    return intra_sc
  # intracell eh matrix
  matrix_sc = get_pwave_matrix(g.r,g.r) # get intracell pairing
