...
def build_honeycomb_bilayer(h,t=0.1):
  ...
  go.x = np.concatenate([x,x+1.])  # move one unit to the right
  go.y = np.concatenate([y,y])  # leave invariant
  ...
  for i in range(len(go.r)): # add the interlayer
    for j in range(len(go.r)): # add the interlayer
      rij = go.r[i] - go.r[j] # distance between atoms
      dz = go.z[i] - go.z[j] # vertical distance
      if (3.9<rij.dot(rij)<4.1) and (3.9<(dz*dz)<4.1): # check if connect
        if ho.has_spin: # spin polarized
          ho.intra[2*i,2*j] = t
          ho.intra[2*i+1,2*j+1] = t
...
def build_honeycomb_trilayer(h,t=0.1):
  """Builds a multilayer based on a hamiltonian, it is assumed that
  there are no new intercell hoppings in the multilayer"""
  g = h.geometry  # save geometry
  ho = deepcopy(h) # copy the hamiltonian
  ...
  # modify the geometry
  x,y,z = np.array(g.x), np.array(g.y) ,np.array(g.z) # store positions
  go.x = np.concatenate([x-1.,x,x+1.])  # move one unit to the right
  go.y = np.concatenate([y,y,y])  # leave invariant
