...
def add_sublattice_imbalance(h,mass):
  ...
  else: return # if does not have sublattice
#    def ab(i): 
#      print("WARNING, system does not have sublattice")
#      return (-1)**i

  if h.has_spin:
    natoms = intra.shape[0]//2 # assume spinpolarized calculation 
  ...
  if not h.has_spin:
    natoms = intra.shape[0] # assume spinpolarized calculation 
    rows = range(natoms)
    ...
    massterm = csc_matrix((data,(rows,rows)),shape=(natoms,natoms))
...
def first_neighbors0d(h):
  ...
  if h.is_sparse: pass # for sparse matrix
    ...
    h.intra = h.intra.todense()  # intracell
    if h.has_spin: # if it has spin degree of freedom
      from increase_hilbert import m2spin
      h.intra = m2spin(h.intra,h.intra) # intracell term 
...
def parametric_hopping(r1,r2,f):
  ...
  return parametric_hopping(r1,r2,f)
