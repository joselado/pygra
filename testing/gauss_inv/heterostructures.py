...
def get_tmatrix(ht,energy=0.0,delta=0.0001):
  ...
  # old code.....
  (selfl,selfr) = get_selfenergies(ht,energy=energy,delta=delta) # selfenergy
  if ht.block_diagonal: raise  # not implemented
  # full central green function
  intra = ht.central_intra
  heff = intra + selfl + selfr
  iden = np.matrix(np.identity(len(intra),dtype=complex)) # create idntity
  gc = ((energy+1j*delta)*iden - heff).I # inverse matrix
  # gamma functions
  gammar = 1j*(selfr-selfr.H)
  gammal = 1j*(selfl-selfl.H)
  # now do the Fisher Lee trick
#  from scipy.linalg import sqrtm # square root of a matrix
  t = sqrtm(gammar)*gc*sqrtm(gammal) # transmission matrix
#  t = sqrtm(gammar)*sqrtm(gammal) # transmission matrix
#  print t
  return t
...
def get_surface_selfenergies(hetero,energy=0.0,delta=0.0001):
