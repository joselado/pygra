...
def hubbard2d(h,mag_ini=None,nk=10):
  """Two dimensional Hubbard mean field calculation"""
  if mag_ini is None: mag_ini = np.random.random(intra.shape[0])-0.5 # random
  ks = []
  for ik in np.linspace(0.,1.,nk):
    for jk in np.linspace(0.,1.,nk):
      ks.append([ik,jk]) # appended kpoints
